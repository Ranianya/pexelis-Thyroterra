...
import api from '../api/axios'; // ✅ Using the helper with baseURL and Interceptors
...
const SignIn = () => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      // ✅ No need for full URL, 'api' uses http://localhost:5000/api
      ...
      // ✅ Navigate to the dashboard (ensure this route exists in App.jsx)
      navigate('/dashboard'); 
    } catch (err) {
      // If server is down, err.response won't exist
      if (!err.response) {
    ...
    <section className='bg-[#64805d] font-pixel min-h-screen flex items-center justify-center p-4'>
      <div className='flex w-full max-w-6xl h-[600px] overflow-hidden rounded-3xl shadow-2xl bg-[#5a7254]'>
        ...
        <div className='hidden md:block w-1/2 relative'>
          <img src="/signin.png" alt="Thyroterra World" className='w-full h-full object-cover' />
        ...
        <div className='w-full md:w-1/2 p-12 flex flex-col justify-center bg-[#64805d] text-white'>
          ...
          {/* Error Alert */}
          {error && (
            <p className="text-red-400 text-[10px] mb-4 bg-red-900/20 p-2 rounded border border-red-400 animate-pulse">
              {error}
            </p>
          )}
          ...
          <form onSubmit={handleSignIn}>
            ...
            <div className='relative mb-6'>
               <div className='absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none'>
                <img src="/password.png" alt="password icon" className='h-5 w-5 opacity-50'/>
              </div>
              <input 
                type="password" 
                required
          ...
          <Link to="/signup" className='w-full text-center border border-[#4a6b32] text-[#5CE96D] py-3 rounded-xl text-sm hover:bg-[#4a6b32]/20 transition-all active:scale-95 mb-8'>
          ...
          <div className='flex flex-col gap-4'>
            <button className='flex items-center justify-center gap-3 bg-[#2d8e00] hover:bg-[#36a800] py-3 rounded-lg transition-all'>
               <img src="/google.png" className='w-5 h-5' alt="google"/><span className='text-sm'>Google</span>
            </button>
          </div>
        </div>
      </div>
