...
const TaskCard = ({ task, toggleComplete, removeTask, index }) => (
  ...
  >
    ...
    <div className="mb-4 text-3xl">{getIconComponent(task.icon)}</div>
  ...
  return (
    <div className="min-h-screen bg-[url('/bg.png')] bg-cover font-pixel p-4 flex flex-col items-center">
      ...
      <header className="w-full max-w-6xl flex items-center justify-between mb-8 gap-4 z-50">
        ...
        <nav className="flex-grow max-w-xl relative">
          <div className="bg-[#f1e4c3] border-[3px] border-[#b89a67] rounded-[40px] px-6 py-2 flex justify-between items-center shadow-[0_8px_0_0_#b89a67] relative">
            <div className="hidden md:flex justify-around w-full">
              {["HOME", "FOREST", "LAND", "PROGRESS", "TODAY", "MONTH", "TASKS"].map((link) => (
                <button key={link} onClick={() => navigate("/" + link.toLowerCase())} className="text-[10px] font-black text-black hover:text-green-700 transition-colors uppercase px-2">{link}</button>
              ))}
            </div>
        ...
        <motion.button whileHover={{ scale: 1.1, rotate: 10 }} onClick={() => { localStorage.clear(); window.location.reload(); }} className="bg-black p-2.5 rounded-lg text-white shadow-[3px_3px_0_0_#5A7554]">
          <RotateCcw size={20}/>
        </motion.button>
      </header>
      ...
      <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-2xl bg-[#f1e4c3] border-4 border-[#b89a67] rounded-xl p-4 mb-8 text-center relative shadow-[0_8px_0_0_#b89a67]">
        ...
        <motion.div key={completedCount} initial={{ scale: 1.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute -right-4 top-12 flex items-center bg-white px-3 py-1 rounded-full border-2 border-stone-400 shadow-md">
      ...
      <div className="w-full max-w-2xl flex flex-col gap-3 mb-8">
        <motion.button whileTap={{ scale: 0.98 }} onClick={() => navigate("/checklist")} className="bg-[#f1e4c3] border-2 border-[#5A7554] text-stone-800 py-2 rounded-lg font-black shadow-[0_4px_0_0_#5A7554] hover:bg-white uppercase text-xs">
          Add to the Routine
        </motion.button>
      ...
      {/* PROGRESS FOOTER */}
      <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="w-full max-w-2xl bg-[#f1e4c3] border-4 border-[#b89a67] rounded-xl p-6 shadow-[0_8px_0_0_#b89a67] mb-10">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="w-full h-10 bg-[#4a3d24] rounded-xl border-4 border-[#b89a67] overflow-hidden flex items-center p-1 relative">
            <motion.div initial={{ width: 0 }} animate={{ width: `${progressPercentage}%` }} className="h-full bg-gradient-to-r from-green-600 to-green-400 rounded-lg shadow-[inset_0_2px_4px_rgba(255,255,255,0.3)]" />
