...
import React, { useState, useEffect } from 'react';
import { Heart, RotateCcw, Smile, Frown, Pill, Droplets, Footprints, Apple, Moon, Menu, X } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
...
const ThyroterraDashboard = () => {
  ...
  const getFinishContent = () => {
    ...
    if (completedCount === 3) return { msg: "🌱 Halfway Through the Journey!", sub: "Your strength grows—keep completing your daily quests!" };
    ...
    return (
      <div className="min-h-screen bg-[url(./bg.png)] bg-cover font-pixel p-4 flex flex-col items-center justify-center text-center">
         <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <h2 className="text-2xl md:text-3xl font-black text-stone-800 mb-6 uppercase">{content.msg}</h2>
          <p className="text-lg font-bold text-stone-700 mb-8">{content.sub}</p>
          <button onClick={() => setIsFinished(false)} className="bg-[#5A7554] text-white py-3 px-8 rounded-full border-4 border-[#3d5238] font-black hover:scale-105 transition-transform uppercase">Continue</button>
        </motion.div>
        
        <button onClick={() => navigate('/monthly-progress')} className="mt-8 bg-[#3d5238] text-white px-6 py-3 rounded-xl font-bold hover:bg-black transition-colors uppercase">Show Monthly Progress</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[url(./bg.png)] bg-cover font-pixel p-4 flex flex-col items-center">
      ...
      {/* HEADER : Logo (Gauche) | Navbar (Milieu) | Reset (Droite) */}
      <header className="w-full max-w-6xl flex items-center justify-between mb-8 gap-4">
        
        {/* Logo */}
        <div className="w-12 h-12 sm:w-16 sm:h-16 flex-shrink-0">
          <img src="./logo.png" alt="logo" className="w-full h-full object-contain cursor-pointer" onClick={() => navigate('/')} />
        </div>

        {/* Navbar Capsule au milieu */}
        <nav className="flex-grow max-w-xl relative">
          ...
          <AnimatePresence>
            {isMenuOpen && (
              ...
              >
                {["HOME", "FOREST", "LAND", "PROGRES","TODAY" ,"MONTH","TASKS"].map((item) => (
                  <button key={item} onClick={() => setIsMenuOpen(false)} className="font-black text-stone-800 text-xs uppercase">{item}</button>
                ))}
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="flex-shrink-0">
          <button
            onClick={() => { if(window.confirm("Reset all tasks?")) { localStorage.clear(); window.location.reload(); } }}
      ...
      <div className="w-full max-w-2xl flex flex-col gap-3 mb-6">
        <button 
          ...
          className="bg-[#f1e4c3] border-2 border-[#5A7554] text-stone-800 py-2 rounded-lg font-bold shadow-[0_4px_0_0_#5A7554] hover:translate-y-0.5 hover:shadow-[0_2px_0_0_#5A7554] transition-all uppercase text-sm"
        >
          {showAddForm ? "✖ Close Form" : "+ Add to the Routine"}
        </button>

        <button 
          onClick={() => setIsFinished(true)}
          className="bg-[#5A7554] border-3 border-[#3d5238] text-white py-3 rounded-lg font-black shadow-[0_4px_0_0_#3d5238] hover:translate-y-0.5 hover:shadow-[0_2px_0_0_#3d5238] transition-all uppercase tracking-wider"
        >
          Finish Tasks
        </button>
      </div>
      ...
      <div className="w-full max-w-2xl bg-[#f1e4c3] border-4 border-[#b89a67] rounded-xl p-6 shadow-[0_8px_0_0_#b89a67]">
        <div className="flex flex-col items-center gap-4">
          <p className="font-black text-lg text-stone-800 uppercase">Your Progress</p>
          <div className="w-full h-8 bg-[#b89a67] rounded-full border-4 border-[#b89a67] overflow-hidden flex items-center px-1">
            <motion.div initial={{ width: 0 }} animate={{ width: `${progressPercentage}%` }} className="h-4 bg-[#6B5B31] rounded-full transition-all duration-700" />
          </div>
          <span className="font-black text-stone-700">{completedCount}/{tasks.length}</span>
        </div>
      </div>
