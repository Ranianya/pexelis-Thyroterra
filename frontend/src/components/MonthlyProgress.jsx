...
const MonthlyProgress = ({ monthlyStats }) => {
  ...
  return (
    <div className="min-h-screen bg-[url('./bg.png')] bg-cover font-pixel p-3 sm:p-4 overflow-x-hidden flex flex-col items-center relative">
      ...
      {/* Title Banner */}
      <div className="w-full max-w-2xl bg-[#f1e4c3] border-4 border-[#89C33D] rounded-xl p-4 mb-8 text-center shadow-[0_8px_0_0_#1DA303] z-10">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-stone-800">
          Your Monthly Progress
        </h1>
      </div>
      ...
      <div className="relative w-full max-w-2xl flex-1 flex items-center justify-center scale-[0.85] sm:scale-100">
        ...
        {stats.map(stat => (
          <div key={stat.id} className={getPositionClasses(stat.position)}>
            <div className=" p-2 rounded-lg mb-1 text-[#046716]">
              {stat.icon}
            </div>
            <span className="text-[10px] font-bold uppercase mb-1">
              {stat.label}
      ...
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 sm:left-6 sm:translate-x-0 md:left-20 flex flex-col items-center gap-1 z-20 scale-[0.9] sm:scale-100">
        <div className="bg-[#f1e4c3] border-2 border-[#d9c5a3] rounded-2xl px-3 py-2 max-w-[180px] text-[11px] sm:text-xs font-bold shadow-md text-center">
            {globalProgress > 80
            ? "Such a great progress this month!"
            : "Keep it up, your forest is growing!"}
        </div>
