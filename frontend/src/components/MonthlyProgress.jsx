...
const MonthlyProgress = ({ monthlyStats }) => {
  ...
  // Variantes pour l'apparition en cascade des stats
    ...
    <div className="min-h-screen bg-[url('./bg.png')] bg-cover font-pixel p-3 sm:p-4 overflow-x-hidden flex flex-col items-center relative">
      ...
      <header className="w-full max-w-6xl flex items-center justify-between mb-8 gap-4 z-50">
        <motion.div 
          ...
          className="w-12 h-12 sm:w-16 sm:h-16 flex-shrink-0 cursor-pointer"
        >
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          {globalProgress > 30 && (
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4 }} className="w-0 h-0 border-l-[32px] border-l-transparent border-r-[32px] border-r-transparent border-b-[42px] border-[#70C593] -mt-4"/>
          )}
          {globalProgress > 60 && (
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.6 }} className="w-0 h-0 border-l-[42px] border-l-transparent border-r-[42px] border-r-transparent border-b-[52px] border-[#65B787] -mt-5"/>
          )}
        ...
        {stats.map(stat => (
          <motion.div 
            ...
            className={getPositionClasses(stat.position)}
          >
            <motion.div 
              animate={{ y: [0, -4, 0] }}
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
              className="p-2 rounded-lg mb-1 text-[#046716]"
            >
              {stat.icon}
            </motion.div>
      ...
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 sm:left-6 sm:translate-x-0 md:left-20 flex flex-col items-center gap-1 z-20 scale-[0.9] sm:scale-100">
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-[#f1e4c3] border-2 border-[#d9c5a3] rounded-2xl px-3 py-2 max-w-[180px] text-[11px] sm:text-xs font-bold shadow-md text-center"
        >
          <motion.p
            initial="hidden"
            animate="visible"
